  ...
  <div class="bg-background-color-darker w-full h-fit mt-44 pt-8 pb-16">
    <div class="w-10/12 mx-auto flex flex-col items-center">
      ...
      <div v-if="activeCategoryArray.length > 0" class="flex gap-4">
        ...
        >
          <div
            v-if="product.offPercent"
            class="bg-gold-brand text-white w-[3.125rem] h-[3.125rem] flex items-center justify-center rounded-full absolute right-5 top-2"
          >
            - {{ product.offPercent }} %
          </div>
          <img
...
export default {
  ...
  computed: {
    ...
    activeCategoryArray() {
      return (
        this.products[this.activeCategory]?.filter((p) => p.offPercent > 0) ||
        []
      );
    },
  },
